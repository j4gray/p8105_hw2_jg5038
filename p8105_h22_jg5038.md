...
Import and clean pols_month data

``` r
pols_df = read_csv(file="./data/fivethirtyeight_datasets/pols-month.csv") |> 
  janitor::clean_names() |> 
  #let's add a lubridate before we separate the date
  mutate(
    lub_date = ymd(mon)
  ) |> 
  separate(
...
The resulting dataset has a dimension of 822, 12 and covers 1947 to
2015. The variables are year, month, gov_gop, sen_gop, rep_gop, gov_dem,
sen_dem, rep_dem, lub_date, president, snp_close, unemployment.
...
mr_tw_df = read_excel('./data/202509 Trash Wheel Collection Data.xlsx', range="Mr. Trash Wheel!A2:N709") |> 
  janitor::clean_names() |> 
  mutate(
    machine = 'mr',
...
prof_tw_df = read_excel('./data/202509 Trash Wheel Collection Data.xlsx', range="Professor Trash Wheel!A2:M134") |> 
  janitor::clean_names() |> 
  mutate(
    machine = 'prof',
  ) |> 
...
gwyn_tw_df = read_excel('./data/202509 Trash Wheel Collection Data.xlsx', range="Gwynns Falls Trash Wheel!A2:L351") |> 
  janitor::clean_names() |> 
  mutate(
    machine = 'gwyn',
  ) |> 
...
This data contains 1188 observations and includes summary data about the
...
Professor Trash Wheel was 2283.84 tons. The total number of cigarette
...
zip_df = read_csv(file="./data/zillow_data/Zip Codes.csv") |> 
  janitor::clean_names() |> 
  #because it will come up later let's manually update the neighborhood of 10069 to Upper West Side
...
``` r
select(filter(zip_df, zip_code=='10069'))
```
...
#10069 initially did not have a neighborhood assigned but by looking on a map we can see it is a small slice of Riverside Drive on the Upper West Side that is completely surrounded by the zip code 10023. 
```
...
|    10069 | Upper West Side               | 4623.042 | 3874.918 |       -748.1245 |
